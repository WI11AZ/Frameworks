...
print(f"Search indices: OPM={len(search_indices['by_opm'])}, WRL={len(search_indices['by_wrl'])}, Name keys={len(search_indices['by_name'])}")

# Save the data as JSON string for the web app
final_data = {
    'app_data': app_data,
...
def test_search(query):
    ...
    # Try OPM search
    if query.isdigit():
        role = search_indices['by_opm'].get(query)
        if role:
            print(f"Found by OPM: {role['name']} ({role['id']})")
            return role
    
    # Try WRL search
    if query.upper() in search_indices['by_wrl']:
        role = search_indices['by_wrl'][query.upper()]
        print(f"Found by WRL: {role['name']} ({role['id']})")
        return role
    
    # Try name search
    if query.lower() in search_indices['by_name']:
        role = search_indices['by_name'][query.lower()]
        print(f"Found by name: {role['name']} ({role['id']})")
        return role
...
# Test cases
test_search("751")
test_search("DD-WRL-001") 
test_search("Cybersecurity Architecture")